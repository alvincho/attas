(function () {
    const STORAGE_KEY = "agent-ui-preferences:v1";
    ...
    function applyPreferences(nextPreferences) {
        ...
        window.dispatchEvent(new CustomEvent("agent:preferenceschange", { detail: prefs }));
        return prefs;
    }
    ...
    async function syncGlobalSettings() {
        try {
            ...
            console.warn("[agentTheme] Global settings sync failed:", error);
        }
    }
    ...
    function applyGlobalSettings(settings) {
        ...
        if (settings.theme || settings.accent) {
            ...
            if (next.theme !== current.theme || next.accent !== current.accent) {
                console.log("[agentTheme] Applying global theme/accent:", next);
        ...
        console.log("[agentTheme] Applied global typography settings");
    }
