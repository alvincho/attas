"""
Regression tests for User Agent.
...
`test_user_agent_generate_can_create_snapshot_without_castr`,
`test_user_agent_generate_can_run_llm_precast_before_cast`,
`test_user_agent_generate_runs_snapshot_and_cast_chain`, and
`test_user_agent_launch_rejects_disabled_agent_config`, helping guard against
...
def test_user_agent_root_serves_dashboard():
    """Exercise the test_user_agent_root_serves_dashboard regression scenario."""
...
def test_user_agent_catalog_aggregates_multi_plaza_results():
    """
    Exercise the test_user_agent_catalog_aggregates_multi_plaza_results regression
    scenario.
    """
    ...
    with patch.object(agent, "_fetch_single_plaza_catalog", side_effect=fake_catalog):
        client = TestClient(agent.app)
        response = client.get("/api/catalog?q=macro")
...
def test_user_agent_catalog_enriches_application_with_host_phemar_address():
    """
    Exercise the
    test_user_agent_catalog_enriches_application_with_host_phemar_address regression
    scenario.
    """
...
def test_user_agent_catalog_hides_disabled_components():
    """
    Exercise the test_user_agent_catalog_hides_disabled_components regression
    scenario.
    """
    agent = build_user_agent(
        plaza_urls=["http://plaza-a"],
        config={
            "user_agent": {
                "disabled_components": {
                    "applications": ["Stock Report"],
                    "phemars": ["StockReportPhemar"],
                    "castrs": ["PdfCastr", "PptCastr"],
                }
            }
        },
    )
            ...
            return [
                {
                    "agent_id": "app-stock",
                    "name": "Stock Report",
                    "description": "Unfinished stock workflow",
                    "owner": "StockReportPhemar",
                    "pit_type": "Phema",
                    "card": {"phema_id": "app-stock", "name": "Stock Report"},
                    "meta": {"host_phemar_name": "StockReportPhemar", "host_phemar_agent_id": "phemar-stock"},
                },
                {
                    "agent_id": "app-macro",
                    "name": "Macro Brief",
                    "description": "Ready workflow",
                    "owner": "MacroPhemar",
                    "pit_type": "Phema",
                    "card": {"phema_id": "app-macro", "name": "Macro Brief"},
                    "meta": {"host_phemar_name": "MacroPhemar", "host_phemar_agent_id": "phemar-macro"},
                },
            ]
        if kwargs.get("role") == "phemar":
            return [
                {
                    "agent_id": "phemar-stock",
                    "name": "StockReportPhemar",
                    "description": "Disabled draft phemar",
                    "role": "phemar",
                    "card": {"name": "StockReportPhemar", "role": "phemar", "address": "http://stock-phemar"},
                },
                {
                    "agent_id": "phemar-macro",
                    "name": "MacroPhemar",
                    "description": "Ready phemar",
                    "role": "phemar",
                    "card": {"name": "MacroPhemar", "role": "phemar", "address": "http://macro-phemar"},
                },
            ]
        if kwargs.get("role") == "castr":
            return [
                {
                    "agent_id": "castr-pdf",
                    "name": "PdfCastr",
                    "description": "Disabled draft castr",
                    "role": "castr",
                    "card": {"name": "PdfCastr", "role": "castr", "address": "http://pdf-castr", "tags": ["pdf"]},
                },
                {
                    "agent_id": "castr-map",
                    "name": "MapCastr",
                    "description": "Ready castr",
                    "role": "castr",
                    "card": {"name": "MapCastr", "role": "castr", "address": "http://map-castr", "tags": ["html"]},
                },
            ]
        return []
    ...
    assert [item["name"] for item in payload["applications"]] == ["Macro Brief"]
    assert [item["name"] for item in payload["phemars"]] == ["MacroPhemar"]
    assert [item["name"] for item in payload["castrs"]] == ["MapCastr"]


def test_user_agent_catalog_marks_stale_plaza_connection_disconnected():
    """
    Exercise the test_user_agent_catalog_marks_stale_plaza_connection_disconnected
    regression scenario.
    """
    agent = build_user_agent(plaza_urls=["http://plaza-a"])
    ...
    assert payload["connected_agent_name"] == "PrompitsUserAgent"
    assert payload["connected_last_active"] == stale_last_active
    assert payload["connection_status"] == "disconnected"


def test_user_agent_agent_config_list_hides_disabled_components():
    """
    Exercise the test_user_agent_agent_config_list_hides_disabled_components
    regression scenario.
    """
    agent = build_user_agent(
        plaza_urls=["http://plaza-a"],
        config={
            "user_agent": {
                "disabled_components": {
                    "phemars": ["StockReportPhemar"],
                    "castrs": ["PdfCastr", "PptCastr"],
                    "agent_configs": ["StockReportPhemar", "PdfCastr", "PptCastr"],
                }
            }
        },
    )
    ...
    with patch.object(
        agent,
        "_fetch_plaza_agent_configs",
        return_value=[
            {"id": "cfg-map", "name": "MapCastr", "role": "castr", "agent_type": "content.castrs.map_castr.MapCastr"},
            {"id": "cfg-pdf", "name": "PdfCastr", "role": "castr", "agent_type": "content.castrs.pdf_castr.PDFCastr"},
            {"id": "cfg-ppt", "name": "PptCastr", "role": "castr", "agent_type": "content.castrs.pptx_castr.PPTXCastr"},
            {"id": "cfg-stock", "name": "StockReportPhemar", "role": "phemar", "agent_type": "content.agents.phemar.PhemarAgent"},
        ],
    ):
        response = client.get("/api/agent_configs")

    assert response.status_code == 200
    payload = response.json()
    assert [item["name"] for item in payload["agent_configs"]] == ["MapCastr"]
    assert [item["name"] for item in payload["plazas"][0]["agent_configs"]] == ["MapCastr"]


def test_user_agent_launch_rejects_disabled_agent_config():
    """
    Exercise the test_user_agent_launch_rejects_disabled_agent_config regression
    scenario.
    """
    agent = build_user_agent(
        plaza_urls=["http://plaza-a"],
    ...
    with (
        ...
        patch.object(agent, "_launch_plaza_agent_config") as launch_mock,
    ):
        response = client.post("/api/agent_configs/launch", json={"config_id": "cfg-pdf", "plaza_url": "http://plaza-a"})

    assert response.status_code == 403
    assert response.json()["detail"] == "This component is temporarily disabled"
    launch_mock.assert_not_called()


def test_primary_plaza_session_uses_latest_base_agent_token():
    """
    Exercise the test_primary_plaza_session_uses_latest_base_agent_token regression
    scenario.
    """
    agent = UserAgent(
        name="PrompitsUserAgent",
    ...
    agent.agent_id = "user-a"
    agent.api_key = "api-key-a"
    agent.plaza_token = "fresh-token"
    agent.token_expires_at = 4321.0
    agent._plaza_sessions["http://plaza-a"] = {
        "plaza_url": "http://plaza-a",
        "agent_id": "user-a",
        "api_key": "api-key-a",
        "token": "stale-token",
    ...
    session = agent._get_session_state("http://plaza-a")

    assert session["agent_id"] == "user-a"
    assert session["api_key"] == "api-key-a"
    assert session["token"] == "fresh-token"
    assert session["token_expires_at"] == 4321.0


def test_primary_plaza_session_clears_stale_cached_token_when_base_token_is_gone():
    """
    Exercise the
    test_primary_plaza_session_clears_stale_cached_token_when_base_token_is_gone
    regression scenario.
    """
    agent = UserAgent(
        name="PrompitsUserAgent",
    ...
    agent.agent_id = "user-a"
    agent.api_key = "api-key-a"
    agent.plaza_token = None
    agent.token_expires_at = 0.0
    agent._plaza_sessions["http://plaza-a"] = {
        "plaza_url": "http://plaza-a",
        "agent_id": "user-a",
        "api_key": "api-key-a",
        "token": "stale-token",
        "token_expires_at": 999999.0,
    }
    ...
    assert session["token_expires_at"] == 0.0


def test_user_agent_lists_remote_snapshots_for_selected_application():
    """
    Exercise the test_user_agent_lists_remote_snapshots_for_selected_application
    regression scenario.
    """
    agent = build_user_agent()
    application = {
    ...
    with (
        ...
        client = TestClient(agent.app)
        response = client.get("/api/snapshots?application_id=app-1")
...
def test_user_agent_lists_remote_snapshots_using_host_address_fallback():
    """
    Exercise the test_user_agent_lists_remote_snapshots_using_host_address_fallback
    regression scenario.
    """
    agent = build_user_agent()
    application = {
    ...
    def fake_fetch_snapshot_history(*, phemar, phema_id, limit, query=""):
        """Return the fake fetch snapshot history."""
...
def test_user_agent_generate_runs_snapshot_and_cast_chain():
    """
    Exercise the test_user_agent_generate_runs_snapshot_and_cast_chain regression
    scenario.
    """
    agent = build_user_agent()
    application = {
...
def test_user_agent_generate_can_create_snapshot_without_castr():
    """
    Exercise the test_user_agent_generate_can_create_snapshot_without_castr
    regression scenario.
    """
    agent = build_user_agent()
    application = {
...
def test_user_agent_generate_can_cast_existing_snapshot_without_new_snapshot_call():
    """
    Exercise the
    test_user_agent_generate_can_cast_existing_snapshot_without_new_snapshot_call
    regression scenario.
    """
    agent = build_user_agent()
    application = {
...
def test_user_agent_generate_can_run_llm_precast_before_cast():
    """
    Exercise the test_user_agent_generate_can_run_llm_precast_before_cast regression
    scenario.
    """
    agent = build_user_agent()
    application = {
...
def test_user_agent_does_not_fall_back_to_removed_llm_practices():
    """
    Exercise the test_user_agent_does_not_fall_back_to_removed_llm_practices
    regression scenario.
    """
    agent = build_user_agent()
    application = {
    ...
    llm_chat_row = {
        ...
        "name": "OpenAIPulser",
        "role": "llm",
        "description": "Legacy LLM practice host",
        "card": {
            "name": "OpenAIPulser",
            "role": "llm",
            "address": "http://openai-pulser",
    ...
    def fake_search(plaza_url, **params):
        """Handle fake search."""
        if params.get("pit_type") == "Pulser" and params.get("pulse_name") == "llm_chat":
            return [llm_chat_row]
        return []

    with patch.object(agent, "_search_plaza", side_effect=fake_search), patch.object(
        agent,
        "_lookup_plaza_agent",
        return_value=llm_chat_row,
    ):
    ...
    assert resolved["practice_id"] == "get_pulse_data"
    assert resolved["pulse_name"] == "llm_chat"


def test_user_agent_can_save_result_locally_and_serve_artifact(tmp_path):
    """
    Exercise the test_user_agent_can_save_result_locally_and_serve_artifact
    regression scenario.
    """
    pool = FileSystemPool(name="user-agent-test", description="user agent test pool", root_path=str(tmp_path))
    ...
    assert saved["local_artifact_url"].startswith("/api/saved_artifacts/")
    ...
    list_response = client.get("/api/saved_results")
