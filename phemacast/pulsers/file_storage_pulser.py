"""
File Storage pulser implementation for the Pulsers area.
...
Core types exposed here include `FileStoragePulser`, which carry the main behavior or
...
SYSTEM_PARTY = "System"
DEFAULT_STORAGE_ROOT = Path(__file__).resolve().parents[2] / "storage" / "file_storage_pulser"
...
class FileStoragePulser(Pulser):
    """Represent a file storage pulser."""
    def __init__(
        self,
        ...
        *,
        ...
        """Initialize the file storage pulser."""
        ...
        resolved_name = str(name or self.raw_config.get("name") or "FileStoragePulser")
        ...
        self._setup_file_storage_routes()

    def _setup_file_storage_routes(self) -> None:
        """Internal helper to set up the file storage routes."""
        @self.app.get("/")
        async def file_storage_pulser_ui(request: Request):
            """Route handler for GET /."""
            return self.templates.TemplateResponse(
                request=request,
                name="phemacast/pulsers/templates/file_storage_pulser_editor.html",
                context={
        ...
        async def get_file_storage_pulser_config():
        ...
        async def save_file_storage_pulser_config(request: Request):
        ...
        async def test_file_storage_pulser_pulse(request: Request):
            """Exercise the test_file_storage_pulser_pulse regression scenario."""
            payload = await request.json()
    ...
    def _save_config_document(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not self.config_path:
            raise HTTPException(status_code=400, detail="This FileStoragePulser was not started from a config file.")
    ...
    def _normalize_storage_config(self, storage_config: Any) -> Dict[str, Any]:
        ...
        normalized["root_path"] = self._normalize_storage_field(raw.get("root_path") or raw.get("path") or "storage/file_storage_pulser")
        return normalized
    ...
    def _normalize_config_document(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        document.setdefault("type", "phemacast.pulsers.file_storage_pulser.FileStoragePulser")
