(function () {
  const STORAGE_KEY = "ads-boss-settings-v1";
  const SIDEBAR_STORAGE_KEY = "ads-boss-sidebar-v1";
  const initial = window.__ADS_BOSS_INITIAL__ || {};
  const state = {
  ...
  const DEFAULT_HERO_METRICS = [
    { id: "security_master", label: "Security Master", table_name: "security_master", count: 0, available: false },
    { id: "daily_price", label: "Daily Price", table_name: "daily_price", count: 0, available: false },
    { id: "news", label: "News", table_name: "news", count: 0, available: false },
    { id: "sec_companyfacts", label: "SEC Companyfacts", table_name: "ads_sec_companyfacts", count: 0, available: false },
    { id: "sec_submissions", label: "SEC Submissions", table_name: "ads_sec_submissions", count: 0, available: false },
  ...
  function heroMetricList(payload) {
    ...
    return DEFAULT_HERO_METRICS.map(metric => merged.get(metric.id) || metric);
  }
    ...
    return `
      <section class="parameter-card" data-param-type="symbols">
        <div class="parameter-card-head">
          <div class="parameter-card-title">Symbols</div>
        </div>
        <div class="field">
          <label for="symbols">Ticker Symbols</label>
          <input id="symbols" name="symbols" type="text" value="${escapeHtml(currentSymbols)}" placeholder="MSFT, NVDA">
        </div>
  ...
  function buildJobImportantSummary(job, options = {}) {
    const symbols = Array.isArray(job?.symbols) ? job.symbols.filter(Boolean) : [];
    ...
    if (symbols.length) {
      ...
      parts.push(symbols.length > 3 ? `Symbols: ${symbolPreview} +${symbols.length - 3}` : `Symbols: ${symbolPreview}`);
    }
    if (job?.target_table) {
    ...
    Object.entries(payloadData)
      .filter(([key, value]) => {
        ...
        return !["symbol", "symbols"].includes(normalizedKey) && value !== "" && value !== null && value !== undefined;
      })
    ...
    return `
      ...
      <section class="job-detail-card">
        ...
        <div class="job-detail-card-header" style="margin-top: 14px; margin-bottom: 8px;">
          <h3 class="job-detail-card-title">Symbols</h3>
        </div>
        ${pillListMarkup(symbols, "No symbols attached to this job.")}
        <div class="job-detail-card-header" style="margin-top: 14px; margin-bottom: 8px;">
  ...
  function renderBossSubmitResult(payload) {
    ...
    shell.innerHTML = `
      ...
      <section class="job-detail-card">
        ...
        ${pillListMarkup(symbols, "No symbols attached.")}
        <div class="job-detail-card-header" style="margin-top: 14px; margin-bottom: 8px;">
          <h3 class="job-detail-card-title">Capability Tags</h3>
        </div>
    ...
    dispatcherShell.innerHTML = `
      <section class="monitor-detail-card">
        <div class="monitor-detail-head">
          <div>
            <h3 class="monitor-detail-title">ADS Dispatcher</h3>
        ...
        ${
          connectionStatus === "not_configured"
            ? `<div class="monitor-alert-list"><div class="monitor-alert">Point the boss at an ADS dispatcher to load queue and worker telemetry.</div></div>`
            : ""
        }
        ${
  ...
  function renderRuntimeSummary() {
    ...
    if (byId("settings-boss-name")) byId("settings-boss-name").textContent = rs.boss_name || "ADS Boss";
  ...
  async function refreshPlazaStatus() {
    try {
      const payload = await fetchJson("/api/plaza/status");
      state.plaza_status = payload || {};
      if (state.plaza_status.plaza_url) {
        state.runtime_summary.plaza_url = state.plaza_status.plaza_url;
      }
      if (state.plaza_status.agent_id) {
        state.runtime_summary.agent_id = state.plaza_status.agent_id;
      }
      renderRuntimeSummary();
  ...
  async function loadHeroMetrics() {
    ...
    try {
      ...
      if (!metricsPayload && !monitorPayload) {
        throw new Error("Unable to load ADS metrics.");
      }
      ...
      state.hero_metrics = Object.assign(
        ...
        state.hero_metrics || initial.hero_metrics || {},
        {
          status: "unreachable",
          dispatcher_address: currentDispatcherAddress(),
          error: error.message || "Unable to load ADS metrics.",
        }
      );
  ...
  async function connectPlaza() {
    ...
    try {
       const payload = await fetchJson("/api/plaza/connect", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ plaza_url: url }),
      });
      state.plaza_status = payload.plaza_status || {};
      state.runtime_summary.plaza_url = url;
      state.runtime_summary.agent_id = state.plaza_status.agent_id || state.runtime_summary.agent_id;
      
      // The shared connector will pick up the change on next poll or we can trigger it
      if (window.attasAgentConnection?.plazaRef) {
          window.attasAgentConnection.plazaRef.refresh();
      }
  ...
  async function handleJobSubmit(event) {
    ...
    try {
      const parsedPriority = Number.parseInt(String(byId("priority")?.value || "100"), 10);
      const option = currentIssueJobOption();
      const symbols = jobOptionRequiresSymbols(option)
  ...
  async function handleScheduleSubmit(event) {
    ...
    try {
      ...
      const payload = {
        ...
        symbols: (byId("schedule-symbols")?.value || "").split(",").map(s => s.trim()).filter(Boolean),
      ...
      <div class="pulse-card${state.selected_job_id === job.id ? " is-active" : ""}" data-job-id="${escapeHtml(job.id)}">
        ...
        <div class="pulse-card-line pulse-card-line-secondary">
          <span class="pulse-card-subtitle">${escapeHtml(
            Array.isArray(job.symbols) && job.symbols.length
              ? job.symbols.join(", ")
              : (buildJobImportantSummary(job, { maxPayloadFields: 1, maxParts: 1 }) || "No symbols attached.")
          )}</span>
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    if (window.attasAgentConnection) {
        window.attasAgentConnection.mountStickyHeader({ selector: '.agent-sticky-header' });
        window.attasAgentConnection.plazaRef = window.attasAgentConnection.mount({
    ...
    if (byId("boss-settings-form")) byId("boss-settings-form").addEventListener("submit", (e) => {
      e.preventDefault();
      ...
      setStatus("settings-status-chip", "Saved Locally", "success");
      syncMonitorAutoRefresh();
      renderRuntimeSummary();
      updateHeroConnectButton({
    ...
    if (byId("settings-save-local")) byId("settings-save-local").addEventListener("click", () => {
        ...
        a.href = url;
        a.download = `ads-boss-settings-${new Date().getTime()}.json`;
        a.click();
