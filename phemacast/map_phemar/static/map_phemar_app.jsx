...
const MINDMAP_BASE_SHAPE_PRESETS = [
  ...
  { id: "branch", label: "Branch", description: "Diamond decision", w: 18, h: 18, ...(MAP_PHEMAR_PRESET_OVERRIDES.branch || {}) },
];
...
const BOOTSTRAP = window.__PHEMACAST_MAP_PHEMAR_BOOTSTRAP__ || window.__PHEMACAST_PERSONAL_AGENT_BOOTSTRAP__ || {};
const APP_MODE = String(BOOTSTRAP?.meta?.app_mode || "").trim().toLowerCase();
const IS_MAP_PHEMAR_MODE = APP_MODE === "map_phemar";
const MAP_PHEMAR_SETTINGS_SCOPE = String(BOOTSTRAP?.meta?.map_phemar_settings_scope || "").trim().toLowerCase() || "map_phemar";
...
const MAP_PHEMAR_USER_GUIDE_PATH = String(BOOTSTRAP?.meta?.user_guide_path || "").trim();
...
const DEFAULT_MAP_PHEMAR_STORAGE_DIRECTORY = "phemacast/map_phemar/storage";
...
function mapPhemarUsesInheritedPlazaConnection() {
  return IS_MAP_PHEMAR_MODE && MAP_PHEMAR_SETTINGS_SCOPE !== "map_phemar";
}

function defaultMapPhemarStorageDirectory() {
  return String(BOOTSTRAP?.settings?.default_file_save_local_directory || DEFAULT_MAP_PHEMAR_STORAGE_DIRECTORY).trim();
}
...
function browserLayoutStorageTarget(preferences, appState) {
  ...
  const directory = String(preferences?.fileSaveLocalDirectory || DEFAULT_MAP_PHEMAR_STORAGE_DIRECTORY).trim();
  return {
    backend: "filesystem",
...
function defaultSaveStorageTarget(preferences, appState) {
  ...
  return {
    backend: "filesystem",
    label: "Local Filesystem",
    detail: directory || DEFAULT_MAP_PHEMAR_STORAGE_DIRECTORY,
  };
}
...
function openMapPhemarUserGuide() {
  if (!MAP_PHEMAR_USER_GUIDE_PATH) {
    return;
  }
  window.open(MAP_PHEMAR_USER_GUIDE_PATH, "_blank", "noopener,noreferrer");
}
...
function mindMapShapePreset(shapeId) {
  ...
  return MINDMAP_SHAPE_PRESETS.find((entry) => entry.id === normalizedShapeId)
    || MINDMAP_BASE_SHAPE_PRESETS.find((entry) => entry.id === normalizedShapeId)
    || MINDMAP_SHAPE_PRESETS.find((entry) => entry.id === "rectangle")
    || MINDMAP_BASE_SHAPE_PRESETS.find((entry) => entry.id === "rectangle")
...
function createMindMapLoadWarning(message, extra = {}) {
  return {
    id: String(extra.id || createId("mindmap-load-warning")),
    code: String(extra.code || "load-warning").trim() || "load-warning",
    targetType: String(extra.targetType || "").trim(),
    targetId: String(extra.targetId || "").trim(),
    message: String(message || "").trim() || "MapPhemar repaired part of the loaded diagram.",
  };
}

function mindMapLoadWarningKey(entry) {
  return [
...
function normalizeMindMapLoadWarnings(entries) {
  const seen = new Set();
  return (Array.isArray(entries) ? entries : [])
    .map((entry) => createMindMapLoadWarning(entry?.message || entry, entry))
    .filter((entry) => {
      const key = mindMapLoadWarningKey(entry);
      if (!entry.message || seen.has(key)) {
        return false;
      }
      seen.add(key);
      return true;
...
function appendMindMapLoadWarning(warnings, message, extra = {}) {
  if (!Array.isArray(warnings)) {
    return;
  }
  const next = createMindMapLoadWarning(message, extra);
  const key = mindMapLoadWarningKey(next);
  if (warnings.some((entry) => mindMapLoadWarningKey(entry) === key)) {
    return;
  }
  warnings.push(next);
}

function noteMindMapLoadWarning(map, message, extra = {}) {
  if (!(map && typeof map === "object")) {
    return;
  }
  const warnings = normalizeMindMapLoadWarnings(map.loadWarnings);
  appendMindMapLoadWarning(warnings, message, extra);
  map.loadWarnings = warnings.slice(0, 24);
}

function normalizeMindMapNode(entry, index = 0, loadWarnings = null) {
  const candidate = entry && typeof entry === "object" ? entry : {};
  const role = normalizeBoundaryMindMapRole(candidate?.role);
  const requestedType = isBoundaryMindMapRole(role) ? "pill" : candidate?.type === "pulse" ? "rectangle" : candidate?.type;
  const preset = mindMapShapePreset(requestedType);
  const isBranch = !isBoundaryMindMapRole(role) && preset.id === "branch";
  const nodeId = candidate?.id || (isBoundaryMindMapRole(role) ? `mind-boundary-${role}` : createId("mind-node"));
  const title = isBoundaryMindMapRole(role)
    ? boundaryMindMapTitle(role)
    : String(candidate?.title || `${preset.label} ${String(index + 1).padStart(2, "0")}`);
  const legacySubtitle = String(candidate?.pulseName || "").trim();
  const inputSchema = candidate?.inputSchema && typeof candidate.inputSchema === "object" ? candidate.inputSchema : {};
  const outputSchema = candidate?.outputSchema && typeof candidate.outputSchema === "object" ? candidate.outputSchema : {};
  if (!(entry && typeof entry === "object")) {
    appendMindMapLoadWarning(loadWarnings, `Node ${index + 1} was not a JSON object, so MapPhemar rebuilt it as a placeholder node.`, {
      code: "node-not-object",
      targetType: "node",
      targetId: String(nodeId),
    });
  }
  if (!isBoundaryMindMapRole(role) && candidate?.type && candidate.type !== "pulse" && preset.id !== String(candidate.type || "").trim()) {
    appendMindMapLoadWarning(loadWarnings, `Node "${title}" used unsupported shape "${String(candidate.type || "").trim()}" and was kept as a ${preset.label.toLowerCase()}.`, {
      code: "unsupported-node-shape",
      targetType: "node",
      targetId: String(nodeId),
    });
  }
  return {
    ...
    type: preset.id,
    title,
    subtitle: isBoundaryMindMapRole(role) || isBranch ? "" : String(candidate?.subtitle || (legacySubtitle && legacySubtitle !== title ? legacySubtitle : "")),
    body: isBranch ? "" : String(candidate?.body || ""),
    x: Number.isFinite(candidate?.x) ? candidate.x : clamp(12 + (index % 4) * 18, 0, 78),
    y: Number.isFinite(candidate?.y) ? candidate.y : clamp(12 + Math.floor(index / 4) * 18, 0, 80),
    w: Number.isFinite(candidate?.w) ? candidate.w : preset.w,
    h: Number.isFinite(candidate?.h) ? candidate.h : preset.h,
    pulserMode: String(candidate?.pulserMode || "specific"),
    pulserId: isBranch ? "" : String(candidate?.pulserId || ""),
    pulserName: isBranch ? "" : String(candidate?.pulserName || ""),
    pulserAddress: isBranch ? "" : String(candidate?.pulserAddress || ""),
    practiceId: String(candidate?.practiceId || "get_pulse_data"),
    pulseName: isBoundaryMindMapRole(role) || isBranch ? "" : String(candidate?.pulseName || ""),
    pulseAddress: isBoundaryMindMapRole(role) || isBranch ? "" : String(candidate?.pulseAddress || ""),
    inputSchema,
    outputSchema,
    inputSchemaText: typeof candidate?.inputSchemaText === "string" ? candidate.inputSchemaText : schemaTextValue(inputSchema),
    outputSchemaText: typeof candidate?.outputSchemaText === "string" ? candidate.outputSchemaText : schemaTextValue(outputSchema),
    inputSchemaError: typeof candidate?.inputSchemaError === "string" ? candidate.inputSchemaError : "",
    outputSchemaError: typeof candidate?.outputSchemaError === "string" ? candidate.outputSchemaError : "",
    paramsText: typeof candidate?.paramsText === "string" ? candidate.paramsText : "{}",
    conditionExpression: isBranch ? String(candidate?.conditionExpression || candidate?.expression || "") : "",
    branchInputSide: normalizeBranchConnectorSide(candidate?.branchInputSide, defaultBranchConnectorSide("input")),
    branchYesSide: normalizeBranchConnectorSide(candidate?.branchYesSide, defaultBranchConnectorSide("yes")),
    branchNoSide: normalizeBranchConnectorSide(candidate?.branchNoSide, defaultBranchConnectorSide("no")),
    branchInputMode: normalizeBranchConnectorMode(candidate?.branchInputMode, defaultBranchConnectorMode("input")),
    branchYesMode: normalizeBranchConnectorMode(candidate?.branchYesMode, defaultBranchConnectorMode("yes")),
    branchNoMode: normalizeBranchConnectorMode(candidate?.branchNoMode, defaultBranchConnectorMode("no")),
  };
}

function normalizeMindMapEdge(entry, index = 0, loadWarnings = null) {
  const candidate = entry && typeof entry === "object" ? entry : {};
  const edgeId = candidate?.id || createId("mind-edge");
  if (!(entry && typeof entry === "object")) {
    appendMindMapLoadWarning(loadWarnings, `Edge ${index + 1} was not a JSON object, so MapPhemar rebuilt it as a placeholder link.`, {
      code: "edge-not-object",
      targetType: "edge",
  ...
  return {
    ...
    fromAnchor: String(candidate?.fromAnchor || ""),
    toAnchor: String(candidate?.toAnchor || ""),
    mappingText: typeof candidate?.mappingText === "string" ? candidate.mappingText : "{}",
    route: normalizeBranchMindMapRoute(candidate?.route || candidate?.branchRoute || candidate?.decision || candidate?.fromAnchor),
  };
}
...
async function chooseLocalDirectory(initialDirectory = "") {
  ...
  if (!response.ok) {
    if (response.status === 404) {
      throw new Error("Folder picker route is not available on the running MapPhemar backend yet. Restart the MapPhemar app, then try Choose Folder again.");
    }
...
function fileStoragePulserListBucketPulse(pulser) {
  return (pulser?.supported_pulses || []).find((pulse) => (
    String(pulse?.pulse_name || pulse?.name || "").trim().toLowerCase() === "list_bucket"
  )) || null;
}

function fileStoragePulserBrowseBucketPulse(pulser) {
  return (pulser?.supported_pulses || []).find((pulse) => (
    String(pulse?.pulse_name || pulse?.name || "").trim().toLowerCase() === "bucket_browse"
  )) || null;
}

function fileStoragePulserCreateBucketPulse(pulser) {
  return (pulser?.supported_pulses || []).find((pulse) => (
    String(pulse?.pulse_name || pulse?.name || "").trim().toLowerCase() === "bucket_create"
  )) || null;
}
...
function samePulserSelection(left, right) {
  if (!left || !right) {
    return false;
  }
  return (
...
function normalizeBucketCatalogEntries(entries) {
  const deduped = new Map();
  (Array.isArray(entries) ? entries : []).forEach((entry) => {
    const bucketName = String(entry?.bucket_name || entry?.name || "").trim();
    if (!bucketName) {
      return;
    }
    const dedupeKey = bucketName.toLowerCase();
    if (deduped.has(dedupeKey)) {
      return;
    }
    deduped.set(dedupeKey, {
      bucket_name: bucketName,
      visibility: String(entry?.visibility || "private").trim().toLowerCase() || "private",
      owner_agent_id: String(entry?.owner_agent_id || "").trim(),
      owner_agent_name: String(entry?.owner_agent_name || "").trim(),
    });
  });
  return Array.from(deduped.values()).sort((left, right) => (
    String(left?.bucket_name || "").localeCompare(String(right?.bucket_name || ""), undefined, { sensitivity: "base" })
  ));
}
...
function normalizeMapPhemaRecord(phema, existing = null) {
  ...
  const existingSource = existing && typeof existing === "object" ? cloneValue(existing) : {};
  delete source._storage;
  delete existingSource._storage;
...
async function loadMapPhemaLibraryFromConfiguredStorage(appState) {
  ...
  async function loadObjectKey(objectKey) {
    ...
    }), "Unable to load the saved Phema library.");
...
async function saveMapPhemaLibraryToConfiguredStorage(library, appState) {
  ...
  }), "Unable to save the Phema library.");
  return target;
}
...
async function fetchMapPhemaLibrary(query = "", appState = null) {
  ...
  const phemas = await loadLocalFilesystemMapPhemaEntries(target.directory);
  return filterMapPhemaRows(phemas, query);
}
...
async function fetchMapPhema(phemaId, appState = null) {
  ...
  const phemas = await loadLocalFilesystemMapPhemaEntries(target.directory);
  const phema = phemas.find((entry) => (
    String(entry?.phema_id || entry?.id || "").trim() === normalizedPhemaId
    || String(entry?._storage?.file_name || "").trim() === normalizedPhemaId
    || String(entry?._storage?.relative_path || "").trim() === normalizedPhemaId
  )) || null;
  if (!(phema && typeof phema === "object")) {
    throw new Error(`Phema '${normalizedPhemaId}' was not found.`);
  }
  return phema;
}
...
async function saveMapPhemaPayload(phema, appState = null) {
  ...
  const existingPhemas = await loadLocalFilesystemMapPhemaEntries(target.directory);
  const existing = existingPhemas.find((entry) => (
    String(entry?.phema_id || entry?.id || "").trim() === String(phema?.phema_id || phema?.id || "").trim()
  )) || null;
  const saved = normalizeMapPhemaRecord(phema, existing);
  const fileName = String(existing?._storage?.entry_count === 1 ? existing?._storage?.file_name : "").trim()
    || defaultMapPhemaFileName(saved);
  const file = await saveLocalJsonFilePayload({
    directory: target.directory,
    file_name: fileName,
    title: saved.name || "diagram-phema",
    content: cloneValue(saved),
  });
  return normalizeLocalFilesystemMapPhemaRecord(saved, {
    ...file,
...
async function listLocalJsonFilesPayload(filePayload) {
  ...
  const payload = await loadJsonResponse(response);
  if (!response.ok) {
    throw new Error(payload?.detail || payload?.message || "Unable to list local JSON files.");
  }
  return Array.isArray(payload?.files) ? payload.files : [];
}

function isDiagramBackedMapPhema(candidate) {
  const diagram = candidate?.meta?.map_phemar?.diagram;
  return Boolean(diagram && typeof diagram === "object" && !Array.isArray(diagram));
}

function mapPhemaFileTimestamp(candidate) {
  const timestamp = Date.parse(
    String(candidate?.updated_at || candidate?.created_at || candidate?._storage?.modified_at || "").trim(),
  );
  return Number.isFinite(timestamp) ? timestamp : 0;
}

function defaultMapPhemaFileName(phema) {
  const namePart = slugifyFileSaveName(phema?.name || "diagram-phema");
  const idPart = slugifyFileSaveName(phema?.phema_id || phema?.id || createId("phema"));
  return namePart === idPart ? `${namePart}.json` : `${namePart}-${idPart}.json`;
}

function normalizeLocalFilesystemMapPhemaRecord(phema, fileRecord, entryIndex = 0, entryCount = 1) {
  const source = phema && typeof phema === "object" ? cloneValue(phema) : {};
  delete source._storage;
  const fileName = String(fileRecord?.file_name || "").trim();
  const relativePath = String(fileRecord?.relative_path || fileName).trim();
  const fileStem = fileName.replace(/\.json$/i, "").trim();
  const modifiedAt = String(fileRecord?.modified_at || "").trim();
  const fallbackId = fileStem
    ? `${fileStem}${entryCount > 1 ? `-${entryIndex + 1}` : ""}`
    : createId("phema");
  const phemaId = String(source.phema_id || source.id || fallbackId).trim();
  const createdAt = String(source.created_at || modifiedAt || new Date().toISOString());
  const updatedAt = String(source.updated_at || modifiedAt || createdAt);
  return {
    ...source,
    phema_id: phemaId,
    id: phemaId,
    name: String(source.name || fileStem || phemaId || "Diagram Phema").trim() || "Diagram Phema",
    created_at: createdAt,
    updated_at: updatedAt,
    _storage: {
      backend: "filesystem",
      directory: String(fileRecord?.directory || "").trim(),
      file_name: fileName,
      relative_path: relativePath,
      path: String(fileRecord?.path || "").trim(),
...
function localFilesystemMapPhemaScanDirectories(directory) {
  const primaryDirectory = String(directory || "").trim();
  const normalizedPrimary = primaryDirectory.replace(/[\\/]+$/g, "");
  const legacyDirectory = /(?:^|[\\/])pool[\\/]phemas$/i.test(normalizedPrimary)
    ? normalizedPrimary
    : joinLocalFilesystemPath(normalizedPrimary, "pool/phemas");
  return Array.from(new Set([normalizedPrimary, legacyDirectory].filter(Boolean)));
}

async function loadLocalFilesystemMapPhemaEntries(directory) {
  const entries = [];
  for (const scanDirectory of localFilesystemMapPhemaScanDirectories(directory)) {
    const files = await listLocalJsonFilesPayload({ directory: scanDirectory });
    for (const file of files) {
      const fileName = String(file?.file_name || "").trim();
      if (!fileName || fileName.toLowerCase() === "_schema.json") {
        continue;
      }
      try {
        ...
        const library = extractMapPhemaLibrary(loaded?.content);
        const phemas = Array.isArray(library?.phemas) ? library.phemas.filter(isDiagramBackedMapPhema) : [];
        phemas.forEach((entry, index) => {
          entries.push(normalizeLocalFilesystemMapPhemaRecord(entry, {
            ...file,
            ...loaded,
            directory: scanDirectory,
            relative_path: file.relative_path || loaded.file_name,
          }, index, phemas.length));
        });
      } catch (error) {
        console.warn(`Unable to inspect local MapPhema file '${fileName}'.`, error);
      }
    }
  }
  const deduped = new Map();
  entries.forEach((entry) => {
    const key = String(entry?.phema_id || entry?.id || entry?._storage?.path || "").trim();
    if (!key) {
      return;
    }
    const existing = deduped.get(key);
    if (!existing || mapPhemaFileTimestamp(entry) >= mapPhemaFileTimestamp(existing)) {
...
function mapPhemaStorageTarget(preferences, appState = null) {
  ...
  const directory = String(
    preferences?.fileSaveLocalDirectory
      ...
      || DEFAULT_MAP_PHEMAR_STORAGE_DIRECTORY,
  ).trim();
  return {
    backend: "filesystem",
    label: `${scopeLabel} · Local Filesystem`,
    detail: directory ? `${directory.replace(/[\\/]+$/, "")}/*.json` : "*.json",
    directory,
  };
}
...
async function saveWorkspaceLayoutLibraryToConfiguredStorage(library, appState) {
  ...
  if (target.backend === "file_storage_pulser") {
    ...
    }), "Unable to save the workspace layout.");
    return target;
  }
...
async function loadWorkspaceLayoutLibraryFromConfiguredStorage(appState) {
  ...
  if (target.backend === "file_storage_pulser") {
    ...
    }), "Unable to load the saved workspace layout.");
...
async function saveBrowserSnapshotLibraryToConfiguredStorage(library, appState) {
  ...
  if (target.backend === "file_storage_pulser") {
    ...
    }), "Unable to save the pane layouts.");
    return target;
  }
...
async function loadBrowserSnapshotLibraryFromConfiguredStorage(appState) {
  ...
  if (target.backend === "file_storage_pulser") {
    ...
    }), "Unable to load the saved pane layouts.");
...
function createDefaultPreferences(dashboard) {
  return {
    ...
    fileSaveLocalDirectory: String(dashboard.settings?.default_file_save_local_directory || DEFAULT_MAP_PHEMAR_STORAGE_DIRECTORY),
...
function effectiveMindMapPlazaUrl(source, preferences, dashboard) {
  const inheritedUrl = normalizePlazaUrl(preferences?.connectionPlazaUrl || dashboard?.meta?.plaza_url || "");
  const mapUrl = normalizePlazaUrl(source?.mapState?.plazaUrl || source?.plazaUrl || "");
  if (mapPhemarUsesInheritedPlazaConnection() && inheritedUrl) {
    return inheritedUrl;
  }
  return mapUrl || inheritedUrl;
}
...
function normalizeMindMapState(candidate, preferences, dashboard) {
  const loadWarnings = normalizeMindMapLoadWarnings(candidate?.loadWarnings);
  if (candidate?.nodes !== undefined && !Array.isArray(candidate?.nodes)) {
    appendMindMapLoadWarning(loadWarnings, "The loaded diagram nodes were not an array, so MapPhemar rebuilt the canvas with boundary nodes only.", {
      code: "nodes-not-array",
    });
  }
  if (candidate?.edges !== undefined && !Array.isArray(candidate?.edges)) {
    appendMindMapLoadWarning(loadWarnings, "The loaded diagram edges were not an array, so MapPhemar preserved the nodes and cleared the broken edge list.", {
      code: "edges-not-array",
    });
  }
  const normalizedNodes = ensureBoundaryMindMapNodes(Array.isArray(candidate?.nodes)
    ? candidate.nodes.map((entry, index) => normalizeMindMapNode(entry, index, loadWarnings))
    : []);
  const seenNodeIds = new Map();
  normalizedNodes.forEach((node) => {
    const nodeId = String(node?.id || "").trim();
    if (!nodeId) {
      node.id = createId("mind-node");
      appendMindMapLoadWarning(loadWarnings, `A node was missing an id, so MapPhemar assigned "${node.id}".`, {
    ...
    const seenCount = Number(seenNodeIds.get(nodeId) || 0);
    if (!seenCount) {
      seenNodeIds.set(nodeId, 1);
      return;
    }
    const nextId = `${nodeId}__dup${seenCount + 1}`;
    appendMindMapLoadWarning(loadWarnings, `Node "${node.title || nodeId}" had a duplicate id "${nodeId}", so MapPhemar renamed this copy to "${nextId}".`, {
      code: "duplicate-node-id",
      targetType: "node",
      targetId: nextId,
    });
    node.id = nextId;
    seenNodeIds.set(nodeId, seenCount + 1);
  });
  const normalizedMap = {
    nodes: normalizedNodes,
    edges: Array.isArray(candidate?.edges)
      ? candidate.edges.map((entry, index) => normalizeMindMapEdge(entry, index, loadWarnings))
      : [],
  };
  const seenEdgeIds = new Map();
  normalizedMap.edges.forEach((edge) => {
    const edgeId = String(edge?.id || "").trim();
    if (!edgeId) {
      edge.id = createId("mind-edge");
      appendMindMapLoadWarning(loadWarnings, `An edge was missing an id, so MapPhemar assigned "${edge.id}".`, {
        code: "missing-edge-id",
        targetType: "edge",
        targetId: String(edge.id),
      });
      return;
    }
    const seenCount = Number(seenEdgeIds.get(edgeId) || 0);
    if (!seenCount) {
      seenEdgeIds.set(edgeId, 1);
      return;
    }
    const nextId = `${edgeId}__dup${seenCount + 1}`;
    appendMindMapLoadWarning(loadWarnings, `Edge "${edge.label || edgeId}" had a duplicate id "${edgeId}", so MapPhemar renamed this copy to "${nextId}".`, {
      code: "duplicate-edge-id",
      targetType: "edge",
      targetId: nextId,
    });
    edge.id = nextId;
    seenEdgeIds.set(edgeId, seenCount + 1);
  });
  const availableNodeIds = new Set(normalizedMap.nodes.map((entry) => String(entry?.id || "").trim()).filter(Boolean));
  normalizedMap.edges.forEach((edge) => {
    const missingTargets = [];
    if (edge.from && !availableNodeIds.has(edge.from)) {
      missingTargets.push(`source "${edge.from}"`);
    }
    if (edge.to && !availableNodeIds.has(edge.to)) {
      missingTargets.push(`destination "${edge.to}"`);
    }
    if (missingTargets.length) {
      appendMindMapLoadWarning(loadWarnings, `Edge "${edge.label || edge.id}" is still loaded, but it references missing ${missingTargets.join(" and ")}. Review this edge in the inspector and reconnect it if needed.`, {
  ...
  return {
    ...
    loadWarnings: loadWarnings.slice(0, 24),
  };
}
...
function createMapPhemarInitialState(dashboard) {
  ...
  return {
    ...
    preferences,
    settingsDraft: null,
    settingsBucketOptions: [],
    settingsBucketStatus: "idle",
    settingsBucketError: "",
    settingsAddBucketOpen: false,
    settingsNewBucketName: "",
    ...
    (workspace?.windows || []).forEach((windowItem) => {
      if (windowItem?.type === "browser") {
        (windowItem.panes || []).forEach((pane) => {
          ...
          try {
            pane.mindMapState = deserializeMindMapFromPhema(phema, appState.preferences, appState.dashboard);
            pane.title = String(phema.name || pane.title || "Diagram");
          } catch (error) {
            noteMindMapLoadWarning(
              pane.mindMapState || (pane.mindMapState = createDefaultMindMapState(appState.preferences, appState.dashboard)),
              `Unable to refresh linked Phema "${linkedId}": ${String(error?.message || error || "Unknown load error")}`,
              { code: "linked-phema-load", targetType: "node" },
            );
            console.error(`Unable to apply linked MapPhemar state for pane ${pane.id}.`, error);
          }
        });
        return;
      }
      ...
      try {
        windowItem.mindMapState = deserializeMindMapFromPhema(phema, appState.preferences, appState.dashboard);
        windowItem.title = String(phema.name || windowItem.title || "Diagram");
        windowItem.subtitle = "Diagram-backed Phema";
      } catch (error) {
        noteMindMapLoadWarning(
          windowItem.mindMapState || (windowItem.mindMapState = createDefaultMindMapState(appState.preferences, appState.dashboard)),
          `Unable to refresh linked Phema "${linkedId}": ${String(error?.message || error || "Unknown load error")}`,
          { code: "linked-phema-load", targetType: "node" },
        );
        console.error(`Unable to apply linked MapPhemar state for window ${windowItem.id}.`, error);
      }
    });
  });
}
...
function applyReturnedMapPhemaToContext(appState, context, phema) {
  ...
  let restored;
  try {
    restored = deserializeMindMapFromPhema(phema, appState.preferences, appState.dashboard);
  } catch (error) {
    console.error("Unable to apply the returned MapPhemar diagram.", error);
    const targetMap = context?.browserWindowId && context?.paneId
      ? findPaneLocation(appState, context.browserWindowId, context.paneId)?.pane?.mindMapState
      : findWindowLocation(appState, String(context?.windowId || "").trim())?.windowItem?.mindMapState;
    noteMindMapLoadWarning(targetMap, `Unable to apply the returned MapPhemar diagram: ${String(error?.message || error || "Unknown load error")}`, {
      code: "returned-phema-load",
    });
    return false;
  }
  const title = String(phema?.name || "").trim();
  if (context?.browserWindowId && context?.paneId) {
    ...
    if (paneLocation?.pane?.type === "mind_map") {
      paneLocation.pane.mindMapState = cloneValue(restored);
      paneLocation.pane.title = String(title || paneLocation.pane.title || "Diagram");
      return true;
    }
  }
  ...
  windowLocation.windowItem.mindMapState = cloneValue(restored);
  windowLocation.windowItem.title = String(title || windowLocation.windowItem.title || "Diagram");
  windowLocation.windowItem.subtitle = "Diagram-backed Phema";
  return true;
}
...
function App() {
  ...
  function currentSettingsPreferences(currentState = state) {
    return normalizePreferences(
      cloneValue(currentState?.settingsDraft || currentState?.preferences || {}),
      currentState?.dashboard || state.dashboard,
    );
  }

  function openSettingsModal(tab = "") {
    updateState((next) => {
      const fallbackTab = IS_MAP_PHEMAR_MODE ? "storage" : (next.settingsTab || "profile");
      next.settingsOpen = true;
      next.settingsTab = String(tab || fallbackTab).trim() || fallbackTab;
      next.settingsDraft = cloneValue(next.preferences);
      next.settingsBucketOptions = [];
      next.settingsBucketStatus = "idle";
      next.settingsBucketError = "";
      next.settingsAddBucketOpen = false;
      next.settingsNewBucketName = "";
      next.settingsError = "";
      next.settingsSaveStatus = "idle";
      next.settingsLlmSelectedId = next.preferences.llmDefaultConfigId || next.preferences.llmConfigs[0]?.id || next.settingsLlmSelectedId || "";
    });
  }
  ...
  function updateSettingsDraft(mutator) {
    updateState((next) => {
      const draft = currentSettingsPreferences(next);
      mutator(draft);
      next.settingsDraft = normalizePreferences(draft, next.dashboard);
      next.settingsBucketError = "";
      next.settingsError = "";
      next.settingsSaveStatus = "idle";
    });
  ...
  async function chooseSettingsLocalDirectory() {
    ...
    try {
      const selectedDirectory = await chooseLocalDirectory(currentSettingsPreferences().fileSaveLocalDirectory);
      if (!selectedDirectory) {
        return;
      }
      updateSettingsDraft((draft) => {
        draft.fileSaveLocalDirectory = selectedDirectory;
      });
    } catch (error) {
      updateState((next) => {
        next.settingsError = error.message || "Unable to choose a local folder.";
      });
    }
  }
...
function normalizeDockedWindowBounds(windowItem, metrics, preferredBounds = null) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        await refreshSettingsBucketOptions(settingsPreferences, {
          isCancelled: () => cancelled,
        });
      } catch (error) {
        if (!cancelled) {
          console.error("Unable to refresh settings buckets.", error);
        }
      }
    })();
    return () => {
    ...
    state.settingsOpen,
    state.settingsTab,
    state.settingsDraft?.fileSaveBackend,
    state.settingsDraft?.fileSavePulserId,
    state.settingsDraft?.fileSavePulserName,
    state.settingsDraft?.fileSavePulserAddress,
    state.settingsDraft?.connectionPlazaUrl,
    state.globalPlazaStatus.status,
    state.globalPlazaStatus.pulserCount,
    state.globalPlazaStatus.plazaUrl,
  ]);
  ...
  useEffect(() => {
    if (!mapPhemarUsesInheritedPlazaConnection()) {
      return;
    }
    const inheritedPlazaUrl = normalizePlazaUrl(state.preferences.connectionPlazaUrl || state.dashboard.meta?.plaza_url || "");
    updateState((next) => {
      let changed = false;
      next.workspaces.forEach((workspaceEntry) => {
        workspaceEntry.windows.forEach((windowItem) => {
          if (windowItem.type !== "mind_map") {
            return;
          }
          const source = resolveMindMapSource(next, windowItem.id);
          if (!source?.mapState) {
            return;
          }
          if (normalizePlazaUrl(source.mapState.plazaUrl || "") !== inheritedPlazaUrl) {
            source.mapState.plazaUrl = inheritedPlazaUrl;
            changed = true;
          }
          const currentCatalogUrl = normalizePlazaUrl(
            source.windowLocation.windowItem.mindMapCatalog?.plazaUrl
            || source.windowLocation.windowItem.mindMapCatalog?.plaza_url
            || "",
          );
          if (currentCatalogUrl !== inheritedPlazaUrl) {
            source.windowLocation.windowItem.mindMapCatalog = emptyCatalog(inheritedPlazaUrl);
            changed = true;
          }
        });
      });
  ...
  }, [state.preferences.connectionPlazaUrl, state.dashboard.meta?.plaza_url, state.workspaces]);
  ...
  async function refreshSettingsBucketOptions(preferences = currentSettingsPreferences(), options = {}) {
    const normalizedPreferences = normalizePreferences(preferences, state.dashboard);
    const isCancelled = typeof options.isCancelled === "function" ? options.isCancelled : () => false;
    const baseAppState = options.appStateOverride || {
      ...state,
    ...
    if (!options.skipStatusUpdate && !isCancelled()) {
      updateState((next) => {
        if (!next.settingsOpen) {
          return;
        }
    ...
    try {
      const prepared = await ensureStoragePulserReady(baseAppState);
      if (isCancelled()) {
      ...
      if (!pulser) {
        ...
        return { appState: prepared.appState, pulser: null, buckets: [] };
      }
      ...
      const buckets = normalizeBucketCatalogEntries(payload?.buckets);
      updateState((next) => {
        if (!next.settingsOpen) {
          return;
        }
        ...
        if (!samePulserSelection(pulser, livePulser)) {
          return;
        }
        const bucketNames = buckets.map((entry) => String(entry?.bucket_name || "").trim()).filter(Boolean);
        const nextBucketName = bucketNames.includes(String(livePreferences.fileSaveBucketName || "").trim())
          ? String(livePreferences.fileSaveBucketName || "").trim()
          : (bucketNames[0] || "");
        const nextDraft = {
          ...livePreferences,
          fileSavePulserId: pulser.agent_id || livePreferences.fileSavePulserId || "",
          fileSavePulserName: pulser.name || livePreferences.fileSavePulserName || "",
          fileSavePulserAddress: pulser.address || livePreferences.fileSavePulserAddress || "",
          fileSaveBucketName: nextBucketName,
        };
        next.settingsDraft = normalizePreferences(nextDraft, next.dashboard);
        next.settingsBucketOptions = buckets;
        next.settingsBucketStatus = "ready";
        next.settingsBucketError = "";
      });
      return {
  ...
  async function createSettingsBucket() {
    const draftPreferences = currentSettingsPreferences();
    const bucketName = String(state.settingsNewBucketName || "").trim();
    if (!bucketName) {
      updateState((next) => {
        next.settingsBucketError = "Enter a bucket name before adding it.";
      });
      return;
    }

    updateState((next) => {
      next.settingsBucketStatus = "creating";
      next.settingsBucketError = "";
    });

    try {
      const normalizedPreferences = normalizePreferences(draftPreferences, state.dashboard);
      ...
      const nextPreferences = normalizePreferences({
        ...
        fileSavePulserId: pulser.agent_id || normalizedPreferences.fileSavePulserId || "",
        fileSavePulserName: pulser.name || normalizedPreferences.fileSavePulserName || "",
        fileSavePulserAddress: pulser.address || normalizedPreferences.fileSavePulserAddress || "",
        fileSaveBucketName: bucketName,
      }, state.dashboard);
      updateState((next) => {
        const nextDraft = currentSettingsPreferences(next);
        nextDraft.fileSavePulserId = nextPreferences.fileSavePulserId;
        nextDraft.fileSavePulserName = nextPreferences.fileSavePulserName;
        nextDraft.fileSavePulserAddress = nextPreferences.fileSavePulserAddress;
        nextDraft.fileSaveBucketName = bucketName;
        next.settingsDraft = normalizePreferences(nextDraft, next.dashboard);
        next.settingsAddBucketOpen = false;
        next.settingsNewBucketName = "";
      });
      await refreshSettingsBucketOptions(nextPreferences, {
        appStateOverride: {
          ...prepared.appState,
      ...
      updateState((next) => {
        next.settingsBucketStatus = "idle";
        next.settingsBucketError = error.message || "Unable to create the bucket.";
      });
    }
  }

  async function verifyFileStorageBucketOnPulser(preferences) {
    const normalizedPreferences = normalizePreferences(preferences, state.dashboard);
    const bucketName = String(normalizedPreferences.fileSaveBucketName || "").trim();
    if (!bucketName) {
      throw new Error("Enter a bucket name before saving settings.");
    }
    ...
    if (listBucketPulse) {
      ...
      }), "Unable to verify the selected storage bucket.");
      const buckets = Array.isArray(payload?.buckets) ? payload.buckets : [];
      const match = buckets.find((entry) => String(entry?.bucket_name || "").trim() === bucketName) || null;
      if (!match) {
        throw new Error(`Bucket "${bucketName}" was not found on ${pulserName}.`);
      }
      return {
        appState: prepared.appState,
        pulser,
    ...
    const browseBucketPulse = fileStoragePulserBrowseBucketPulse(pulser);
    if (browseBucketPulse) {
      try {
        unwrapStoragePulserPayload(await runPulserRequest({
          ...requestBase,
          pulse_name: browseBucketPulse.pulse_name || browseBucketPulse.name || "bucket_browse",
          pulse_address: browseBucketPulse.pulse_address || "",
          output_schema: browseBucketPulse.output_schema || {},
          input: {
            bucket_name: bucketName,
            limit: 1,
          },
  ...
  async function saveSettingsModal() {
    const draftPreferences = currentSettingsPreferences();
    updateState((next) => {
      next.settingsSaveStatus = "saving";
      next.settingsError = "";
    });
    try {
      ...
      if (normalizeFileSaveBackend(nextPreferences.fileSaveBackend) === "file_storage_pulser") {
        ...
        nextPreferences.fileSavePulserAddress = String(verified.pulser?.address || nextPreferences.fileSavePulserAddress || "");
      }
      updateState((next) => {
        next.preferences = normalizePreferences(nextPreferences, next.dashboard);
        next.settingsDraft = null;
        next.settingsOpen = false;
        next.settingsError = "";
        next.settingsSaveStatus = "idle";
      ...
      updateState((next) => {
        next.settingsError = error.message || "Unable to save settings.";
        next.settingsSaveStatus = "idle";
      });
    }
  }
  ...
  async function refreshSettingsStorageCatalog() {
    const settingsPreferences = currentSettingsPreferences();
    const normalizedPreferences = normalizePreferences(settingsPreferences, state.dashboard);
  ...
  async function refreshMindMapCatalog(windowId) {
    ...
    const plazaUrl = effectiveMindMapPlazaUrl(source, state.preferences, state.dashboard);
    updateState((next) => {
  ...
  async function refreshMapPhemarPlaza() {
    const primaryWindow = activeWindowList().find((entry) => entry.type === "mind_map") || null;
    if (primaryWindow) {
      await refreshMindMapCatalog(primaryWindow.id);
      return;
    }
    await refreshGlobalPlaza();
  }
  ...
  async function savePaneResultToDefaultLocation(windowId, paneId) {
    ...
    try {
      ...
      if (normalizeFileSaveBackend(state.preferences.fileSaveBackend) === "file_storage_pulser") {
        ...
        }), "Unable to save the pane result.");
  ...
  function createMindMapNodeDraft(map, x, y, shapeId = "rectangle") {
  ...
  function addMindMapNode(windowId, x, y, shapeId = "rectangle") {
    updateMindMap(windowId, (map) => {
  ...
  function selectMindMapEdge(windowId, edgeId) {
    updateMindMap(windowId, (map) => {
      map.selectedNodeId = "";
  ...
  function clearMindMapLoadWarnings(windowId) {
    updateMindMap(windowId, (map) => {
      map.loadWarnings = [];
    });
  }
  ...
  const primaryMapCatalogStatus = primaryMapWindow?.mindMapCatalog?.status || "idle";
  const mapPhemarPlazaBusy = state.globalPlazaStatus.status === "loading" || primaryMapCatalogStatus === "loading";
  ...
  function handleCanvasMove(event) {
    ...
    const loadWarnings = normalizeMindMapLoadWarnings(map.loadWarnings);
    ...
    const contextMeta = selectedNode
      ...
      : selectedEdgeMeta
        ...
        : `${map.nodes.length} nodes and ${map.edges.length} links${loadWarnings.length ? ` · ${loadWarnings.length} load note${loadWarnings.length === 1 ? "" : "s"}` : ""}`;
    function reviewMindMapLoadWarning(warning) {
      if (warning?.targetType === "edge" && warning?.targetId) {
        selectMindMapEdge(windowItem.id, warning.targetId);
        return;
      }
      if (warning?.targetType === "node" && warning?.targetId) {
        selectMindMapNode(windowItem.id, warning.targetId);
      }
    }
        ...
        <div className="mindmap-app-shell">
          <div className="mindmap-layout" onMouseMove={handleCanvasMove} onMouseUp={handleCanvasUp}>
            ...
            <aside className="mindmap-right-panel" style={{ width: `${inspectorWidth}px` }}>
              ...
              {loadWarnings.length ? (
                <div className="mindmap-load-warning-card">
                  <div className="mindmap-load-warning-head">
                    <div>
                      <strong>Load Notes</strong>
                      <span>MapPhemar kept every node and edge it could recover. Review the flagged items here instead of resetting the workspace.</span>
                    </div>
                    <button className="mindmap-ui-button mindmap-ui-button--compact" onClick={() => clearMindMapLoadWarnings(windowItem.id)}>
                      Dismiss
                    </button>
                  </div>
                  <div className="mindmap-load-warning-list">
                    {loadWarnings.slice(0, 6).map((warning) => (
                      <div key={warning.id} className="mindmap-load-warning-item">
                        <span>{warning.message}</span>
                        {warning.targetId ? (
                          <button
                            className="mindmap-ui-button mindmap-ui-button--compact"
                            onClick={() => reviewMindMapLoadWarning(warning)}
                          >
                            {warning.targetType === "edge" ? "Review Edge" : "Review Node"}
                          </button>
                        ) : null}
                      </div>
                    ))}
                    {loadWarnings.length > 6 ? (
                      <p className="compatibility-copy">+{loadWarnings.length - 6} more load note{loadWarnings.length - 6 === 1 ? "" : "s"} still attached to this diagram.</p>
                    ) : null}
                  </div>
                </div>
              ) : null}
            ...
            >
              <div
                ...
                onDrop={(event) => {
                  ...
                  const shapeId = MINDMAP_SHAPE_PRESETS.some((entry) => entry.id === droppedShape) ? droppedShape : "rectangle";
    ...
    return (
      <div className="modal-backdrop" onClick={busy ? undefined : closePhemaDialog}>
        <div className="modal-shell narrow" onClick={(event) => event.stopPropagation()}>
          ...
          <div className="connection-card workspace-dialog-storage-card">
            <strong>{
              storageTarget.backend === "filesystem"
                ? (phemaDialog.mode === "save" ? "Current Save Folder" : "Current Load Folder")
                : (phemaDialog.mode === "save" ? "Current Save Location" : "Current Library Location")
            }</strong>
            <span>{storageTarget.label}</span>
            <span className="workspace-dialog-storage-path">{storageTarget.detail}</span>
          </div>
  ...
  function renderSettingsModal() {
    ...
    const settingsBusy = state.settingsSaveStatus === "saving";
    ...
    const bucketSelectPlaceholder = !selectedStoragePulser
      ...
      : state.settingsBucketStatus === "loading"
        ...
        : settingsBucketOptions.length
          ? "Choose bucket"
          : "No buckets yet";
    const mapPhemaStorageInfo = mapPhemaStorageTarget(settingsPreferences, settingsAppState);
    const configuredStorageSummary = formatStorageTargetSummary(defaultSaveStorageTarget(settingsPreferences, settingsAppState));
    return (
      <div className="modal-backdrop" onClick={settingsBusy ? undefined : cancelSettingsModal}>
        <div className="settings-shell" onClick={(event) => event.stopPropagation()}>
          <div className="settings-sidebar">
            <div className="identity-card">
              <strong>{settingsPreferences.profileDisplayName}</strong>
              <span>{settingsPreferences.profileEmail}</span>
              <p>{settingsPreferences.profileDesk}</p>
            </div>
          ...
          <div className="settings-content">
            ...
            {selectedSettingsTab === "storage" ? (
              <div className="settings-grid">
                {IS_MAP_PHEMAR_MODE && !mapPhemarHasOwnSettings ? (
                  ...
                  <>
                    <div className="settings-storage-backend-row wide">
                      <label className="field settings-storage-backend-field">
                        <span>Default Save Backend</span>
                        <select
                          value={normalizeFileSaveBackend(settingsPreferences.fileSaveBackend)}
                          onChange={(event) => updateSettingsDraft((draft) => { draft.fileSaveBackend = normalizeFileSaveBackend(event.target.value); })}
                        >
                      ...
                      {settingsUsesFileStoragePulser ? (
                        <div className="settings-storage-backend-actions">
                          <button
                            className="ghost-button"
                            onClick={refreshSettingsStorageCatalog}
                            disabled={settingsBusy || settingsCatalogBusy}
                          >
                            {settingsCatalogBusy ? "Refreshing..." : "Refresh Plaza Catalog"}
                          </button>
                        </div>
                      ) : null}
                    </div>
                      ...
                      <>
                        <div className="settings-storage-directory-row wide">
                          <label className="field settings-storage-directory-field">
                            ...
                            <input
                              value={settingsPreferences.fileSaveLocalDirectory}
                              placeholder={IS_MAP_PHEMAR_MODE ? "phemacast/map_phemar/storage" : "phemacast/personal_agent/storage/saved_files"}
                              onChange={(event) => updateSettingsDraft((draft) => { draft.fileSaveLocalDirectory = event.target.value; })}
                            />
                          </label>
                          <div className="settings-storage-directory-actions">
                            <button className="ghost-button" onClick={chooseSettingsLocalDirectory} disabled={settingsBusy}>
                              Choose Folder
                            </button>
                          </div>
                        </div>
                        <div className="connection-card wide">
                          ...
                          <span>{settingsPreferences.fileSaveLocalDirectory || (IS_MAP_PHEMAR_MODE ? "phemacast/map_phemar/storage" : "phemacast/personal_agent/storage/saved_files")}</span>
                        </div>
                      </>
                    ) : (
                      <>
                        <label className="field wide">
                          ...
                          <select
                            ...
                            onChange={(event) => updateSettingsDraft((draft) => {
                              ...
                              draft.fileSavePulserId = pulser?.agent_id || "";
                              draft.fileSavePulserName = pulser?.name || "";
                              draft.fileSavePulserAddress = pulser?.address || "";
                        ...
                        <div className="settings-storage-bucket-row wide">
                          <label className="field settings-storage-bucket-field">
                            <span>Bucket Name</span>
                            <select
                              value={settingsPreferences.fileSaveBucketName || ""}
                              onChange={(event) => updateSettingsDraft((draft) => { draft.fileSaveBucketName = event.target.value; })}
                              disabled={!selectedStoragePulser || settingsBucketBusy}
                            >
                              <option value="">{bucketSelectPlaceholder}</option>
                              {settingsBucketOptions.map((bucket) => {
                                const bucketName = String(bucket?.bucket_name || "").trim();
                                return (
                          ...
                          <div className="settings-storage-bucket-actions">
                            <button
                              className="ghost-button"
                              onClick={() => updateState((next) => {
                                next.settingsAddBucketOpen = !next.settingsAddBucketOpen;
                                next.settingsNewBucketName = "";
                                next.settingsBucketError = "";
                              })}
                              disabled={settingsBusy || !selectedStoragePulser || state.settingsBucketStatus === "loading"}
                            >
                              {state.settingsAddBucketOpen ? "Cancel Add" : "Add New"}
                            </button>
                          </div>
                        </div>
                        {state.settingsAddBucketOpen ? (
                          <div className="settings-storage-bucket-create-row wide">
                            <label className="field settings-storage-bucket-create-field">
                              <span>New Bucket Name</span>
                              <input
                                value={state.settingsNewBucketName}
                                placeholder="demo-assets"
                                onChange={(event) => updateState((next) => {
                                  next.settingsNewBucketName = event.target.value;
                                  next.settingsBucketError = "";
                                })}
                                disabled={settingsBusy || state.settingsBucketStatus === "creating"}
                              />
                            </label>
                            <div className="settings-storage-bucket-create-actions">
                              <button
                                className="accent-button"
                                onClick={createSettingsBucket}
                                disabled={settingsBusy || state.settingsBucketStatus === "creating"}
                              >
                                {state.settingsBucketStatus === "creating" ? "Adding..." : "Add Bucket"}
                              </button>
                            </div>
                          </div>
                        ) : null}
                        <label className="field">
                          ...
                          <input
                            value={settingsPreferences.fileSaveObjectPrefix}
                            placeholder={IS_MAP_PHEMAR_MODE ? "map-phemar/library" : "personal-agent/results"}
                            onChange={(event) => updateSettingsDraft((draft) => { draft.fileSaveObjectPrefix = event.target.value; })}
                          />
                        </label>
                        <div className="connection-card wide">
                          ...
                          {selectedStoragePulser ? (
                            <span>
                              {state.settingsBucketStatus === "loading"
                                ? "Loading the current bucket list from this pulser."
                                : settingsBucketOptions.length
                                  ? `${settingsBucketOptions.length} bucket${settingsBucketOptions.length === 1 ? "" : "s"} currently available on this pulser.`
                                  : "No buckets exist on this pulser yet. Use Add New to create one."}
                            </span>
                          ) : null}
                        </div>
                        {state.settingsBucketError ? <div className="form-error settings-storage-feedback wide">{state.settingsBucketError}</div> : null}
                      </>
                    )}
                    <div className="connection-card wide">
                      {IS_MAP_PHEMAR_MODE ? (
            ...
            {state.settingsError ? <div className="form-error">{state.settingsError}</div> : null}
            <div className="modal-actions">
              <button className="ghost-button" onClick={cancelSettingsModal} disabled={settingsBusy}>Cancel</button>
              <button className="accent-button" onClick={saveSettingsModal} disabled={settingsBusy}>
        ...
        <div className="map-phemar-shell">
          <header className="map-phemar-header">
            ...
            <div className="map-phemar-actions">
              ...
              <button
                className={mapPhemarPlazaBusy ? "ghost-button active" : "ghost-button"}
                onClick={refreshMapPhemarPlaza}
                disabled={mapPhemarPlazaBusy}
              >
                {mapPhemarPlazaBusy ? "Refreshing Plaza..." : "Refresh Plaza"}
              </button>
              {MAP_PHEMAR_USER_GUIDE_PATH ? (
                <button className="ghost-button" onClick={openMapPhemarUserGuide}>
                  User Guide
                </button>
              ) : null}
              {mapPhemarHasOwnSettings ? (
                <button className="ghost-button" onClick={() => openSettingsModal("storage")}>
                  Settings
                </button>
        ...
        <main className="stage">
          <header className="menu-bar shell-panel">
            <div className="menu-group">
              ...
              {MAP_PHEMAR_USER_GUIDE_PATH ? (
                <button className="ghost-button" onClick={openMapPhemarUserGuide}>User Guide</button>
              ) : null}
              <button className="accent-button" onClick={() => openSettingsModal()}>Settings</button>
            </div>
