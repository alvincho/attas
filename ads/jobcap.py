"""
Job-capability interfaces and adapters for `ads.jobcap`.
...
DEFAULT_EMPTY_TABLE_BACKFILL_YEARS = 3


def subtract_years(value: date, years: int) -> date:
    """Handle subtract years."""
    if years <= 0:
        return value
    ...
    try:
        ...
        # Clamp leap-day dates onto Feb 28 for non-leap target years.
        return value.replace(year=target_year, month=2, day=28)


def resolve_daily_price_start_date(
    latest_stored_date: date | None,
    configured_start_date: date | None,
    configured_end_date: date,
    *,
    bootstrap_years: int = DEFAULT_EMPTY_TABLE_BACKFILL_YEARS,
) -> date:
    """Resolve the daily price start date."""
    if latest_stored_date is not None:
        return latest_stored_date
    if configured_start_date is not None:
        return configured_start_date
    return subtract_years(configured_end_date, bootstrap_years)
...
def build_job_cap_map(entries: Iterable[Mapping[str, Any] | JobCap | str] | Mapping[str, Any] | None) -> Dict[str, JobCap]:
